...
// (IDs are context-local) and/or trigger driver instability.
// We use this to safely free backend bookkeeping without issuing glDelete*.
static bool g_ImGui_ImplOpenGL3_SkipGLDeletes = false;

void ImGui_ImplOpenGL3_SetSkipGLDeletes(bool skip)
{
    g_ImGui_ImplOpenGL3_SkipGLDeletes = skip;
}
...
static void ImGui_ImplOpenGL3_DestroyTexture(ImTextureData* tex)
{
    ...
    if (!g_ImGui_ImplOpenGL3_SkipGLDeletes)
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
    ...
    bool was_create_or_update = (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates);
    if (was_create_or_update)
    {
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        ImGui_ImplOpenGL3_Data* bd_es = ImGui_ImplOpenGL3_GetBackendData();
        for (ImTextureRect& r : tex->Updates)
        {
            const int src_pitch = r.w * tex->BytesPerPixel;
            bd_es->TempBuffer.resize(r.h * src_pitch);
            char* out_p = bd_es->TempBuffer.Data;
            for (int y = 0; y < r.h; y++, out_p += src_pitch)
                memcpy(out_p, tex->GetPixelsAt(r.x, r.y + y), src_pitch);
            IM_ASSERT(out_p == bd_es->TempBuffer.end());
            GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, GL_RGBA, GL_UNSIGNED_BYTE, bd_es->TempBuffer.Data));
        }
#endif
        tex->SetStatus(ImTextureStatus_OK);
